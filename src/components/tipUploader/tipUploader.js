...
const createTip = async input => {
  console.log(input);
  await API.graphql(
    graphqlOperation(mutations.createTip, {
      input: input
    })
  ).then(result => {
    console.log("SUCCESS: uploaded tip to DynamoDB");
    console.log(result);
    return true;
  });
};
...
const formikEnhancer = withFormik({
  ...
  handleSubmit: (values, { setSubmitting, props }) => {
    const payload = {
      ...
      tipSubjectId: props.subjectId,
      votes: 0
    };
    let uploaded = createTip(payload);
    if (uploaded) {
