...
const createTypeOfData = async input => {
  await API.graphql(
    graphqlOperation(mutations.createDataType, {
      input: input
    })
  ).then(result => {
    console.log("SUCCESS: uploaded data type to DynamoDB");
    console.log(result);
    return true;
  });
};
...
const formikEnhancer = withFormik({
  validationSchema: Yup.object().shape({
    typeOfData: Yup.string()
      .min(3, "too short")
      .required("Please insert a type of data")
  }),
  ...
  handleSubmit: (values, { setSubmitting, props }) => {
    const input = {
      type: values.typeOfData,
      dataTypeSubjectsId: props.subjectId,
      endorsements: [props.user.email]
    };
    console.log(input);
    let uploaded = createTypeOfData(input);
    setSubmitting(false);
    if (uploaded) {
 ...
 */
const TypeOfDataUploader = props => {
...
export default TypeOfDataUploader;
