...
//dictionary of states used to convert initials to full name
const states = {
  AL: "Alabama",
  AK: "Alaska",
  AZ: "Arizona",
...
class SearchWarrant extends React.Component {
  ...
  downloadWarrant = event => {
    const filename =
      this.props.prefix != null
        ? `${this.props.prefix}${this.props.id}`
        : this.props.id;
  ...
  updateVotes = async (newVoteCount, message) => {
  ...
  highLightWords = (excerpt, keywordsList) => {
    const regex = /[.,\s]/g;
  ...
  renderHighlights = excerpt => {
    ...
    return this.highLightWords(fullExcerpt, keywords);
  };

  reduceSentence = (content, quantity) => {
  ...
  render() {
    ...
    let validCounty = this.props.county ? true : false;
    let validState = this.props.state ? true : false;
    var seal = validState
      ? require("../../assets/seals/" + this.props.state + ".png")
      : null;
    ...
    let crimeList = [];
    let crimeListLength = Array.isArray(this.props.crimes)
      ? this.props.crimes.length
      : 0;
    let fullList = false;
    for (let i = 0; i < crimeListLength; i++) {
      let crime = this.props.crimes[i];
    ...
    //handle isTemplate
    let isTemplate = this.props.isTemplate ? true : false;
    ...
    let RenderExcerpt = () => {
      if (this.props.content != null) {
        ...
        let contentInfo = this.renderHighlights(this.props.content);
        let excerptContent = contentInfo[0];
        let keywords = contentInfo[1];
        excerptContent[0] = this.reduceSentence(excerptContent[0], 30);
        return (
          <p>
            {excerptContent.map(word => {
    ...
    return (
      <SearchWarrantContainer>
        <TopContainer>
          <RankingBlock>
            {!this.props.isModerator && (
              <Vote
                ...
                upvote={() =>
                  this.updateVotes(parseInt(this.state.votes) + 1, "+1")
                }
                downvote={() =>
                  this.updateVotes(parseInt(this.state.votes) - 1, "-1")
                }
          ...
          <InfoBlock>
            <div className="stateContainer">
              {validCounty && <h6>County of {this.props.county}</h6>}{" "}
            </div>
          ...
          <StatePictureBlock>
            <img src={seal} alt={this.props.state} />
          </StatePictureBlock>
        </TopContainer>

        <ExcerptContainer onClick={this.downloadWarrant} download>
        ...
        <BottomContainer>
          <DateElement>
            {this.dateToDays(this.props.uploadTimestamp)}
          </DateElement>
          {!this.props.isModerator && (
            <RelavanceScore relevancyColor={relevancyColor}>
              {Math.round(this.props.relavance * 100) / 100}
            </RelavanceScore>
          )}
        </BottomContainer>
