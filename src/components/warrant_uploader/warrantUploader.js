...
const uploadDocToS3 = async (filename, content, type, setSubmitting) => {
  const path = "unapproved/" + filename;
  ...
  Storage.put(path, content, {
    ...
    .then(result => {
      console.log("(1) SUCCESS: uploaded file to S3 for conversion");
      console.log(result);
      setSubmitting(false);
    })
...
const uploadJoinsToDynamoDB = async (id, subjectIds) => {
  var curSub = 1;
  subjectIds.map(sub =>
    API.graphql(
      ...
      .then(result => {
        console.log(
          `(2) SUCCESS: linked warrant to ${curSub}/${subjectIds.length} subjects`
        );
        console.log(result);
        curSub++;
      })
...
const uploadMetadataToDynamoDB = async (warrant, id, subjectIds) => {
  API.graphql(
    graphqlOperation(mutations.createUnapprovedWarrant, { input: warrant })
  )
    .then(result => {
      console.log("(3) SUCCESS: uploaded metadata to DynamoDB");
      console.log(result);
      uploadJoinsToDynamoDB(id, subjectIds);
    })
    .catch(err => console.log(err));
};
...
const formikEnhancer = withFormik({
  ...
  handleSubmit: async (values, { setSubmitting, props }) => {
    ...
    let step2 = uploadMetadataToDynamoDB(warrant, id, values.subjects);
...
const WarrantUploader = props => {
  ...
  useEffect(() => {
    const fetchSubjectsList = async () => {
      API.graphql(graphqlOperation(query.listSubjects, { limit: 5000 })).then(
        results => {
          console.log(results.data.listSubjects.items);
          setSubjects(results.data.listSubjects.items);
        }
      );
    };

    const fetchCrimeList = async () => {
      API.graphql(graphqlOperation(query.listCrimes, { limit: 5000 })).then(
        results => {
          console.log(results.data.listCrimes.items);
          setCrime([{ name: "" }, ...results.data.listCrimes.items]);
        }
      );
    };

    const fetchDataTypes = async () => {
      API.graphql(graphqlOperation(listDataTypes2, { limit: 5000 })).then(
        result => {
          let filteredDataList = filterDuplicates(
            result.data.listDataTypes.items
          );
          console.log(filteredDataList);
          setDataType([{ types: "" }, ...filteredDataList]);
        }
      );
    };
